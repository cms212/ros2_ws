...
public:
  ...
  : Node("arm_position_subscriber")
  {
    auto arm_position_subscriber_callback =
      [this](message_interfaces::msg::ArmPosition::UniquePtr msg) -> void {
        if (msg->arm_positions[0] <= 1024){
            clockwise_ = false;
        }
        else if (msg->arm_positions[0] >= 3072){
            clockwise_ = true;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Set [ID: %d]", id);
      }
      };
    subscription_ =
      this->create_subscription<message_interfaces::msg::ArmPosition>("arm_position", 10, arm_position_subscriber_callback);
  }


private:
  rclcpp::Subscription<message_interfaces::msg::ArmPosition>::SharedPtr subscription_;
